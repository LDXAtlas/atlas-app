...
### Uploads — Move Library / avatar / logo uploads onto signed upload URLs, then restore the 25 MB Library limit
- Today these uploads send the file through a server action. Next.js caps server-action bodies (default 1 MB), so anything over ~1 MB hung in production until 2026-09-22. That fix raised `serverActions.bodySizeLimit` to 4.25 MB (just under Vercel's 4.5 MB request-body cap) and set `MAX_FILE_BYTES` in `src/lib/file-utils.ts` to **4 MB** for Library files, avatars and org logos.
- The fix: mint a signed upload URL in a server action (`createSignedUploadUrl`, service role), upload straight from the browser (`uploadToSignedUrl`), then confirm with a second action that checks the real object size. This is the same pattern Huddles Phase 2 recordings use. After that, restore the Library limit to 25 MB (the DB CHECK) and lower `bodySizeLimit` back.
- **Also affected, not changed on 2026-09-22:** announcement cover images (`uploadAnnouncementCover` + `compose-modal.tsx`) allow 5 MB through a server action. Covers up to ~4 MB now work; 4–5 MB covers still fail at the request limit. Move them in the same pass.

### Huddles Phase 2 — part 1 follow-ups (opened 2026-09-22)
- **The 10-credit floor to start recording is a Lucas call to revisit.** `startHuddleRecording` refuses below `HUDDLE_RECORDING_MIN_CREDITS` (10 credits ≈ 10 minutes of transcription). It's a guess at "enough to be worth starting". Watch what real meetings cost and decide whether to raise it, scale it to scheduled length, or warn instead of refuse.
- **`consume_ai_credits` cannot refuse.** It adds unconditionally and returns `GREATEST(0, limit - used)`, so an org can silently run past its limit. The pre-call checks in `startHuddleRecording` and `transcribeHuddleRecording` are the only guard, and that is true of **every** `callAI()` path too. The security audit should consider a guarded RPC that fails when the balance is short.
- **Part 3 scope** (not built): retention purge job (it must recompute `retention_until` via `recompute_huddle_recording_retention` rather than trust the stored value, and skip `recording_pinned`); storage-quota enforcement + a usage meter (counting already works); playback (signed download URL + player); recovering an in-progress segment after a tab crash (IndexedDB); Realtime for the consent indicator instead of polling; per-segment delete; server-side transcoding for anything over Whisper's 25 MB; a sweep for rows stuck in `pending_upload`.
- **`huddles.status` never moves to `processing` / `ready`.** Both values exist in the live CHECK and part 1 deliberately leaves status alone; part 2 decides whether the summary pipeline drives them.

### Huddles — Phase 1 migration file does not describe the live huddle tables (found 2026-09-22)
- `supabase/migrations/20260615_huddles_phase_1.sql` (and the Phase 0 file's notes) claim to mirror production, but the live huddle tables differ in column names, columns, FK delete rules, CHECK values and policy names. Examples: `huddle_recordings` has `file_type` / `source_type` / `deleted_at` (no `mime_type`); `huddle_transcripts` has `full_text` / `model_used` (no `content` / `word_count`) and already had `recording_id`; `huddle_summaries` has `executive_summary` / `topics_discussed` / `sentiment_notes` / `model_used` / `transcript_id` (no `summary` / `model`); `huddle_decisions` has `decided_at` (no `created_at`); `huddle_attendees.role` allows `organizer / attendee / optional` only (no `presenter`). Where the migration file came from is unknown.
- **Until the security audit reconciles them, `supabase/LIVE_SCHEMA_2026-09-22.md` is the record for huddle-table columns.** Don't take a column name, constraint or policy name from a migration file without checking live. The first Phase 2 migration draft failed on apply for exactly this reason.
- Code that already disagrees with live (found by cross-checking `src/app/actions/huddles.ts` and the export registry against the dump). **None of it is fixed yet:**
  - ~~Probable live bug: `huddle_decisions.created_at` doesn't exist~~ **Confirmed on production and fixed 2026-09-22.** Logging a decision failed silently. The column is `decided_at`; `createDecision`, `getHuddle`, `getRecentDecisions` (now ordered by `decided_at` desc) and the export registry all use it now. `HuddleDecision` keeps its `created_at` field name for the existing UI, filled from `decided_at`. `RecentHuddleDecision`'s field is renamed to `decided_at`.
  - `getHuddle` selects `huddle_transcripts.content` and `huddle_summaries.summary, model`, none of which exist. Those two queries fail silently, which is harmless while the tables are empty. Phase 2 part 1 rewrites the transcript read; the summaries read maps `executive_summary` / `model_used` in the same pass.
  - `AttendeeRole` / `updateAttendeeRole` allow `presenter`, which the live CHECK rejects, so setting someone to presenter fails. **Fix written, not yet applied:** `supabase/migrations/20260922_huddle_attendee_presenter.sql` widens the CHECK (chosen over removing the role, because the dropdown lives in Ben's `attendee-list.tsx` and the role is real — an agenda item's `presenter_id` is a different thing). Nothing in Phase 2 part 1 depends on it.
...
- **The answer to "do the live policies match the migration files?" is already no.** `supabase/migrations/20260615_huddles_phase_1.sql` names the child-table policies "Access child rows for accessible huddles", but live `pg_policies` has "Access huddle recordings via huddle access" and "Access huddle transcripts via huddle access" (found 2026-09-22 while preparing the Phase 2 migration, which now drops them by their live names). The "documentation only" migration files can't be trusted as a record of live RLS.
- Recommendation for the audit: do a one-time dump of every live policy (`SELECT schemaname, tablename, policyname, cmd, roles, qual, with_check FROM pg_policies ORDER BY 1, 2, 3;`) into the repo (e.g. `supabase/live-policies-<date>.sql`). Review that dump instead of the migration files, and diff against it in future audits.
- **Second file-vs-live gap: `private` huddle visibility.** The live huddles SELECT policy shows a `private` huddle to its creator only, and an `invitees_only` huddle to attendees. The app's JS rule (`filterVisibleHuddles`, `loadHuddleForViewer`, `getHuddlesForCalendar`) shows attendees any huddle they're on, **including `private`**. So the app is looser than live RLS. Not changed yet; the audit should decide which rule is right and make both agree.
- **Viewers can write huddle child rows directly.** Live, every huddle child table (`huddle_action_items`, `huddle_agenda_items`, `huddle_attendees`, `huddle_decisions`, `huddle_notes`, `huddle_summaries`, plus recordings/transcripts until the Phase 2 migration is applied) has a `cmd=ALL` policy for anyone who can see the huddle. Any such viewer can insert/update/delete those rows through PostgREST with their own session, bypassing every canEdit/canManage check in the server actions. The audit should narrow all of them to SELECT (writes already go through the service-role client). The Phase 2 migration does this for recordings + transcripts only.

### AI infrastructure — OPENAI_API_KEY must be an "All" key, and the shell can override .env.local
- **Restricted keys cannot do transcription.** Huddle recording's first real Whisper call failed with 401 on every segment. The key was a **Restricted** project key, and OpenAI's restricted-permissions list has **no row for `/v1/audio/transcriptions`** — there is no scope to grant, so no restricted key can ever call it. Fixed 2026-09-22 by setting the key's permissions to **All**. If transcription starts 401-ing again, check this before anything else.
- Atlas reads exactly one OpenAI variable, `OPENAI_API_KEY` (`src/lib/ai/openai-client.ts:69`). No org id, no base URL.
- **A shell export wins over `.env.local`.** Next only fills in variables that aren't already in the environment, so an `OPENAI_API_KEY` exported in the developer's shell (or `~/.zshrc`) silently overrides the repo value under `npx next dev` — and the two keys can have different permissions. Check with `echo ${OPENAI_API_KEY:+set in shell}` before trusting `.env.local`. The same applies to Vercel: its dashboard value is what deployed builds use, and it is a separate key from the local one.
...
### Huddles Phase 2 part 1 — recording, storage, transcription — Completed 2026-09-22
- Backend + two mountable components for browser recording of a huddle, upload to a private bucket, and Whisper transcription. **No huddles UI file touched**; the recorder lives in `src/components/huddle-recorder/` for Ben to mount. Not part 1: the AI summary (part 2), retention purge and quota enforcement (part 3).
- **Migration** `supabase/migrations/20260922_huddles_phase_2_recording.sql`, applied 2026-09-22, written against the live schema (see `supabase/LIVE_SCHEMA_2026-09-22.md` — the Phase 1 file was wrong, and a first draft built from it failed on apply). Private `huddle-recordings` bucket (webm/mp4/ogg, 25 MB, no storage policies so only the service role and signed URLs reach it); `huddle_recordings` became one row per segment; `huddle_transcripts.recording_id` NOT NULL + UNIQUE; `huddle_summaries.source_transcript_ids` and its `transcript_id` FK relaxed to SET NULL; `huddles.recording_state` + heartbeat; a BEFORE INSERT trigger for org id + `retention_until`; `recompute_huddle_recording_retention` and the atomic `adjust_huddle_storage_used` RPCs; RLS on recordings/transcripts narrowed to organizer or org admin; both `notifications` CHECKs extended (`huddle_invited`, `huddle_action_assigned`, `huddle_crisis_flag` — none wired up — plus the `huddle` entity type).
- **Segment model.** Pause stops the recorder and uploads that segment (never `MediaRecorder.pause()`); resume starts a new one; segments also roll at 15 minutes or ~20 MB so each file stays under Whisper's 25 MB limit. Each row stores wall-clock bounds, duration, byte size, the real MIME type in `file_type`, and the bitrate the browser actually used. Codec probe order: `audio/webm;codecs=opus`, `audio/webm`, `audio/mp4` (Safari), `audio/ogg;codecs=opus`, `audio/ogg` (Firefox).
- **Uploads go browser → storage** via `createSignedUploadUrl` / `uploadToSignedUrl`. Server actions can't carry the bytes (1 MB default body limit, 4.25 MB here, 4.5 MB at Vercel). `completeRecordingSegmentUpload` reads the real object size from storage rather than trusting the client, then adds it to `organizations.huddle_storage_used_bytes` through the atomic RPC.
- **Transcription** runs in `POST /api/huddles/recordings/[recordingId]/transcribe` with `maxDuration = 300`, because a server action inherits the timeout of the *page* that calls it — Ben's page. The handler checks the session, then `transcribeHuddleRecording` claims the segment in one UPDATE (so two callers can't both pay Whisper), pre-checks credits, downloads with the service-role client, calls `transcribeAudio()` (never the OpenAI SDK directly), and upserts one transcript row per segment.
- **Credits.** 1 per minute, rounded up per segment, through the existing accounting. Recording never stops mid-meeting for credits: a segment parks as `awaiting_credits` with its audio kept and stays retryable. Starting requires 10 credits (flagged in PENDING as a call to revisit). `transcribeAudio()` now also respects the org `ai_enabled` master switch, which it previously ignored.
- **Transcript visibility.** `getHuddleTranscript` is organizer/admin only, and `getHuddle` no longer returns transcript text or recording paths to other viewers — Phase 1 had returned them to every viewer, and live RLS allowed the same; both reversed before any data existed. `getHuddleRecordingState` is the viewer-safe read (counts and timings, no text) behind the consent indicator.
- **Delete means delete.** `deleteHuddleRecording` removes objects first, then transcripts and rows, then releases the bytes and stamps `recording_deleted_at`; the live `deleted_at` column stays unused. `deleteHuddle` now purges a huddle's audio instead of stranding it in the bucket. `updateHuddleSettings` recomputes `retention_until` when the retention setting changes.
- **Export.** `huddle_transcripts` is registered (parent-scoped via huddles, exporting `full_text`); `huddle_recordings` is a declared exclusion — pointers to a private audio bucket, and v1 exports metadata, never blobs.
...
- **Registry-driven**, mirroring `src/lib/ai/feature-registry.ts`. `src/lib/export/export-registry.ts` is the single source of truth: each entity declares its table, org-scoping strategy, allow-listed fields, and explicitly-redacted fields with reasons. Adding a future module (Serve/Care) = add entries, not rewrite export logic. `EXPORT_SCHEMA_VERSION` (currently 1) stamps every file so old exports stay interpretable.
- **Scoping is declared, never guessed.** Three strategies: `org_root` (the organizations row by `id`), `direct` (`organization_id` column), and `via` (parent-scoped — e.g. board cards via `board_id`, huddle notes via `huddle_id`; the engine collects parent ids and filters children with a chunked `.in()`). This is why schema introspection was rejected — parent-scoped tables have no `organization_id`.
- **Server action** `exportOrganizationData()` in `src/app/actions/data-export.ts`: admin-gated via `can.editOrganization` re-checked server-side (service role bypasses RLS), strictly scoped to the caller's `organization_id`, returns one JSON with a metadata block (org, date, schema_version, per-entity `record_counts`, `redactions`, `excluded_tables`, `warnings`). One failing entity is recorded as a warning and skipped — never fails the whole export.
- **Scope covered:** organization details; staff profiles; departments + staff→department assignments; congregation members (full record incl. `notes`) + member_tags; announcements; tasks (+ comments, + activity); calendar events (+ event_departments, + attendees/RSVPs, + custom_event_types); project boards (columns, cards, checklist items, comments, labels + card-label links, activity log, board members); huddles (attendees, agenda items, notes, decisions, action items) — **data only; no huddles UI touched**; library file **metadata** (not blobs) + folders + tags + file-tag links; AI guidelines (church-authored fields only).
- **Redacted/excluded (declared with reasons, not silent):** `organizations.stripe_customer_id`; `attachments.storage_path`/`thumbnail_path` (blobs out of scope v1); AI guidelines emit only voice_tone/terminology/about_church/things_to_avoid/additional_guidelines (`model_preference`/`ai_enabled` are Atlas operational config, excluded). Whole tables excluded via `EXCLUDED_ORG_TABLES`: `calendar_feed_tokens` (secret ICS tokens), `ai_usage_log` (internal billing/audit), `notifications` (transient), `invitations` (transient + secret accept token).
- **Completeness guard (soft, never fatal):** reads the live schema from the PostgREST OpenAPI doc (`GET /rest/v1/`, service key) — no migration required — and warns in the export metadata about (a) any org-scoped table not in the registry and (b) per-entity field drift (declared field missing live, or live column neither exported nor redacted). Rationale: a silently-incomplete export is the worst failure; a hard failure could block a church from exporting at all. If the schema fetch fails it degrades to a note and still exports.
- **Follow-ups (not v1):** file blobs (bundle the actual Library files, likely a signed-URL manifest or ZIP); CSV-per-entity option if churches ask; Serve/Care entities register here when those modules land. Congregation `members` is included now even though launch is staff-only, so the export stays complete as churches start using it.
...
- **Feature registry** at `src/lib/ai/feature-registry.ts` — single source of truth for AI features (key/displayName/description/minTier/canToggle/usesGuidelines). `huddle_transcription` is the only entry today with `usesGuidelines: false` (Whisper raw text). Unknown keys default to TRUE so any new feature inherits guidelines automatically.
- **Server actions** at `src/app/actions/ai-settings.ts`: `getOrgAISettings()` returns settings + `isAdmin` + `organizationId` (org members can read, falls back to defaults when no row exists); `updateOrgAISettings(input)` admin-gated via `can.editOrganization` (server-side, not just UI), enforces per-field character limits from `AI_SETTINGS_LIMITS` (voice_tone 500, terminology 1000, about_church 1000, things_to_avoid 1000, additional_guidelines 2000), validates model preference + boolean, upserts on PK `organization_id`.
- **Wired into `callAI()`** at `src/lib/ai/index.ts`:
  1. Fetches `getOrgAIContext(organizationId)` per call.
  2. Short-circuits cleanly with a user-facing message when `ai_enabled = false`.
  3. Passes `modelPreference` to `selectModel` (new tier-bounded matrix in `model-selector.ts` — Workspace always Haiku, Suite speed-simple → Haiku else Sonnet, Ultimate quality-complex → Opus). Never lets an org exceed its tier's cost ceiling.
  4. Composes the cached system prefix as `ATLAS_BASE_GUIDELINES` (non-negotiable rules: accuracy, no harmful content, pastoral context not authority, etc.) + the org's guidelines block — with explicit "PREFERENCES, not overrides" framing so jailbreaky org text can't override safety rules.
  5. Skips the org block for features where `featureUsesGuidelines(key)` is false.
  6. Anthropic dispatch sends two system blocks (cached prefix + uncached task prompt) via new `cachedPrefix` param on `callClaude`; OpenAI fallback concatenates them since it takes a single system field. Caching pattern matches the rest of Atlas (`cache_control: { type: 'ephemeral' }`).
- **UI** at `src/app/(app)/settings/ai/` — server page calls `getOrgAISettings`, client form has Guidelines card (5 textareas with character counters + Save/Cancel dirty state), Model Preference card (3 radio cards with tier-aware copy + Workspace upgrade nudge), AI master switch (toggle + banner when off), Insights "coming soon" stub. Read-only banner shown to non-admins; server still enforces the admin gate independently. Added to `/settings` index + shell breadcrumbs.
- **Backward-compatible by design**: orgs with no `organization_ai_settings` row act exactly as they did before (default 'balanced' preference, no guidelines block, AI enabled). The admin test endpoint at `/api/ai/test` that calls `callClaude` without `cachedPrefix` keeps using the single-block legacy path.
- **Future** (not in v1): per-feature toggles (registry already has `canToggle` so the column drop-in is trivial); Insights dashboard (credits used by feature, suggestion acceptance rate, adoption); guidelines templates ("traditional", "modern", "casual"); in-process cache for `getOrgAIContext` (org row changes rarely).
