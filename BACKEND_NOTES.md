...
- Code that already disagrees with live (found by cross-checking `src/app/actions/huddles.ts` and the export registry against the dump). **None of it is fixed yet:**
  - **Probable live bug: `huddle_decisions.created_at` doesn't exist** (the column is `decided_at`), and four places use it: `createDecision` (insert…select), `getHuddle` (decisions select), `getRecentDecisions` (select + order, the rail hook shipped 2026-09-22), and the export registry's `huddle_decisions` fields. If so, logging a decision fails, the detail page silently shows no decisions, the recent-decisions rail errors, and the export warns about field drift. Confirm on production, then fix all four (select `decided_at`; keep the TS field name or rename it deliberately).
